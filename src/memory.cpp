...
void dumpMem(void)
{
    ...
    for (int i = 0; i != 256; i++)
    {
        console_printf("%02x: %s ", i, s_memType[i] == MEM_RAM ? "RAM" : s_memType[i] == MEM_ROM ? "ROM"
                                                                                                 : "UNALLOCATED");
            ...
            else
            {
                ...
                for (int j = 0; j != 256; j += 16)
                {
                    ...
                    for (int k = 0; k != 16; k++)
                    {
                        ...
                        auto c = mem[i * 256 + j + k];
                        console_printf("%c", isgraph(c) ? c : '.');
                    }
                    console_printf("\n");
                }
            }
...
void dumpCore(const char *filename)
{
    ...
    snprintf(corename, sizeof(corename), "core/%s", input);
